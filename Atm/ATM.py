...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.controller.title('LICET_ATM')
        self.controller.state('zoomed')
        p = tk.PhotoImage(file='abc.png')
        self.controller.iconphoto(False,p)

        heading=tk.Label(self,text='LICET ATM',font=('orbitron',45,'bold'),foreground='white',background='#3d3d5c')
        heading.pack(pady=25)

        space_label=tk.Label(self,height=4,bg='#3d3d5c').pack()

        password_label=tk.Label(self,text=(f'Welcome {user_display_name} to LICET Banking'),font=('BatmanForeverAlternate',17),bg='#3d3d5c',fg='white').pack(pady=10)
        ...
        credits=tk.Label(bottom_frame,text='Created and Develped by Licetians',font=('orbitron',15)).pack()
...
class WithdrawPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        heading=tk.Label(self,text='LICET ATM',font=('orbitron',45,'bold'),foreground='white',background='#3d3d5c')
        heading.pack(pady=25)
        choose_amount_label=tk.Label(self,text='Enter the amount you want to withdraw:',font=('orbitron',13),fg='white',bg='#3d3d5c')
        ...
        def withdraw(amount):
            ...
            if amount>current_balance:
                messagebox.showwarning('WARNING','Not sufficient money :(')
                ...
                mydb=mysql.connector.connect(host="localhost",user="root",password="FRBros_2021")
                mycursor=mydb.cursor()
                mycursor.execute("use LICET_ATM")
                mycursor.execute(f"update ATM set balance ={current_balance} where accid = {username1} ")
                mydb.commit()
        ...
        twenty_button=tk.Button(button_frame,text='₹500',font=('orbitron',12),command=lambda:withdraw(500),relief='raised',borderwidth=3,width=30,height=4)
        twenty_button.grid(row=0,column=0,pady=5)

        fourty_button=tk.Button(button_frame,text='₹1000',font=('orbitron',12),command=lambda:withdraw(1000),relief='raised',borderwidth=3,width=30,height=4)
        fourty_button.grid(row=1,column=0,pady=5)

        sixty_button=tk.Button(button_frame,text='₹2000',font=('orbitron',12),command=lambda:withdraw(2000),relief='raised',borderwidth=3,width=30,height=4)
        sixty_button.grid(row=2,column=0,pady=5)

        eighty_button=tk.Button(button_frame,text='₹2500',font=('orbitron',12),command=lambda:withdraw(2500),relief='raised',borderwidth=3,width=30,height=4)
        eighty_button.grid(row=3,column=0,pady=5)

        one_hundred_button=tk.Button(button_frame,text='₹3000',font=('orbitron',12),command=lambda:withdraw(3000),relief='raised',borderwidth=3,width=30,height=4)
        one_hundred_button.grid(row=0,column=1,pady=5,padx=794)

        two_hundred_button=tk.Button(button_frame,text='₹4000',font=('orbitron',12),command=lambda:withdraw(4000),relief='raised',borderwidth=3,width=30,height=4)
        two_hundred_button.grid(row=1,column=1,pady=5)

        three_hundred_button=tk.Button(button_frame,text='₹5000',font=('orbitron',12),command=lambda:withdraw(5000),relief='raised',borderwidth=3,width=30,height=4)
        ...
        other_amount_heading=tk.Button(button_frame,text='Other: (In INR)',font=('orbitron',13),borderwidth=0,relief='sunken',activeforeground='white',activebackground='#33334d',bg='#33334d',fg='white')
    ...
    def __init__(self, parent, controller):
        ...
        def deposit_cash():
            ...
            try:
                val=int(cash.get())
                if int(val)<0:
                    messagebox.showwarning('WARNING','Invalid Amount Entered!')
                    cash.set('')
                else:
...
class InfoPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        mydb=mysql.connector.connect(host="localhost",user="root",password="FRBros_2021")
        mycursor=mydb.cursor()
        mycursor.execute("create database if not exists LICET_ATM")
        mycursor.execute("use LICET_ATM")
        mycursor.execute(f"select password from ATM where accid = {username1} ")
...
def about():
  ...
  screen3.title("About")
  screen3.geometry("500x200+500+500")
  screen3.configure(bg='lightpink')
  p = tk.PhotoImage(file='abc.png')
  screen3.iconphoto(False,p)
  Label(screen3,bg='lightpink', text = "\n\n\n Created and developed by\n SEDRIC \n\n\n Made with: \nTkinter GUI, Python with Mysql database",font = ("orbitron", 10)).pack()
...
def register_user():
  ...
  mycursor.execute("create database if not exists LICET_ATM")
  mycursor.execute("use LICET_ATM")
  mycursor.execute("create table if not exists ATM(accid int(10) primary key,name varchar(30),password char(20),balance char(30))")
  mydb.commit()

  mycursor.execute('select accid from ATM')
  ...
  if username_info in b:
    ...
    name_entry.delete(0,END)
        ...
        name_entry.delete(0, END)
        screen1.destroy()
        mycursor.execute("insert into ATM values('"+username_info+"','"+name_info+"','"+password_info+"','"+balance_inti+"')")
        mydb.commit()
      ...
      if username1.isdigit():
          ...
          if int(username1) in user_acc:
              ...
              if password1=='':
                  ...
                  mycursor.execute(f"select name from ATM where accid={username1}")
                  values=mycursor.fetchall()
                  user_name=[]
                  for i in values:
                  ...
                  mycursor.execute(f'select balance from ATM where accid ={username1}')
                  values=mycursor.fetchall()
                  user_balance=[]
                  for i in values:
...
def main_screen():
  ...
  screen.title("LICET_ATM")
  screen.configure(bg='lightblue')
  p = tk.PhotoImage(file='abc.png')
  screen.iconphoto(False,p)


  Label(text = "LICET ATM",fg='white', bg = "grey", width = "300", height = "2", font = ("orbitron", 15,'bold')).pack()
  Label(text = "",bg='lightblue').pack()

  img = ImageTk.PhotoImage(Image.open("pr_logo.jpg"))
